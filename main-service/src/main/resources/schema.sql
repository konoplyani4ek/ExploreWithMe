...
-- Таблица users отсюда убрана: пользователи теперь живут в БД user-service (ewm-users).
-- initiator_id/requester_id ниже остаются обычными BIGINT без FK — целостность
-- между сервисами обеспечивается на уровне приложения (через вызовы к user-service),
-- а не на уровне БД, т.к. кросс-сервисные внешние ключи невозможны.
